...
- 表 **`ws_host_whitelist`**（见增量脚本）：字段含 **`host_type`**（`engine` / `openclaw`）、**`health_check_url`**、**`online_status`** 等。  
- **Engine**：使用 `host_id` 与 **WebSocket 注册** 绑定，由 `WhitelistService` 等校验。  
- **OpenClaw / Hermes**：由 **`OpenClawHealthChecker`**（`selectHostsForHttpHealthCheck`：`openclaw` / `hermes`）定时 **HTTP GET** 访问 `health_check_url`，**2xx** 视为在线。
