...
**福帮手数据智能化系统**（对外简称 **福帮手 FBSir**，技术标识 **WxFbsir**）是一套面向企业与团队运营场景的 **AI 工具链与智能协同平台**。系统在架构上采用 **主节点（Admin）+ 引擎节点（Engine）+ 可选外部网关（如 OpenClaw Gateway）** 的组合：主节点负责 Web 管理、鉴权、业务编排与持久化；引擎节点通过 **WebSocket** 接入主节点，承载 **Playwright** 浏览器自动化与任务执行；**OpenClaw** 类主机则通过 **HTTP 健康检查** 纳入主机纳管视图。
